...
- **P5 丝印对齐(靠前!)**:`pcb silk-align`(位号摆正+位置感知+`--spacing` 装配间距)。导入的位号常 180° 倒置,这里一并摆正。放布线前,让布线避开丝印占位。
- **P6 可布性门**:`pcb layout-lint`(≥ 目标分、0 overlap、ratsnest 交叉可控)。
- **P7 布线**:`pcb route-short`;个别擦焊盘的手工 `pcb track` 绕行。**注意 `rip-up` 会连电源缝合过孔一起删**——之后重跑 `power-planes` 补回。
- **P8 叠层+电源+铺铜**:`pcb stackup set --layers 4` → `pcb power-planes`(GND内电层+VCC内层+缝合过孔)→ 顶/底 `pcb pour-fit --net GND --replace=false`(`--replace` 默认 true 会清跨层同网铺铜)→ `pour-rebuild`(退让禁布区)。**内层平面用 SIGNAL+铺铜**,别翻 PLANE 类型——PLANE 负平面不自动退让禁布区会报 Plane Zone 错;SIGNAL 铺铜物理等价且自动退让。
- **P9 极性/板注**:`pcb silk-add` 加 LED +/− 极性标(锚点=左下角,放器件外的空白处别压焊盘/本体)、板注 credit;`pcb silk-set --ref board --align centerx` 居中。
- **P10 门**:`pcb drc`(passed)+ `pcb check`(0 issue)双清零,再 `pcb save`。
