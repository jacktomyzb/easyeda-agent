...
- `pcb.drc.rules` — read the active PCB's **DRC rule configuration** (clearances, track widths, via sizes, …) **without running a check**. Use to feed real rule values into layout reasoning / gates, or to see what `pcb.drc.check` enforces. The daemon parses the (deeply-nested, untyped) result into `{clearance, trackWidth, trackWidthMin, viaDrill, viaDiameter}` in mil (`internal/app/pcb_rules.go`); `route-short`/`auto-place` consume it so they conform to the board's spec.
  > **Writing rules** (no typed command yet — debug path): `eda.pcb_Drc.overwriteCurrentRuleConfiguration(cfg.config)` works but **takes the BARE config content** — `getCurrentRuleConfiguration()` returns `{name, config}`, and passing that whole wrapper **silently no-ops** (resolves `undefined`, readback unchanged). Pass `cfg.config` → returns `true`, and the PCB's config becomes `自定义配置` (the system `JLCPCB Capability(...)` configs are immutable per API docs). This is also the validated fix for the **fresh-PCB pour-reflow divergence** (new PCB's pour hugs pads at ~9.7mil with no thermal spokes while a sibling PCB in the same project reflows fine): bump `config.Plane` `lineClearance` 0.254→0.3048 mm (10→12mil) in `copperRegion` (both pad models) + `innerPlane`, overwrite, then `pcb pour-rebuild` — on ceshi/PCB2 this cleared **21 pour-to-pad clearance errors AND 6 GND no-connection (missing thermal) errors** in one shot (DRC 28→1; the remainder is the known add-component netlist false positive).
