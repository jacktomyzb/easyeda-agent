...
- `easyeda pcb silk-align` — **align component designators (位号)** to a consistent spot above (`--side top`, default) or below the footprint bbox, `--offset` mil away. Cleans up the scattered/overlapping designators a fresh import leaves. `--refs U1 --refs LED1` limits to specific parts. The designator is a **component-bound attribute** (`pcb_PrimitiveAttribute`, NOT a free `pcb_PrimitiveString` — that getAll is empty), repositioned via `getAllPrimitiveId(componentId)` + `modify(id,{x,y})`; the component itself has no per-designator position setter. Verify with `pcb snapshot`.
