...
- `easyeda pcb silk-set` — **batch-adjust existing silk** (designators + free strings): `--ids '[...]'` (primitiveIds from `pcb check --json` / a silk list) + any of `--x/--y/--rotation/--font-size/--line-width/--text`; only the given keys change. The batch position/orientation/size fixer for badly-placed or non-upright silk. Uses the reliable `.modify(id,props)` — **rotation persists but a `pcb snapshot` before a document reload shows the OLD orientation (stale render); judge by `pcb check`/silk list, not a screenshot** (setState_Rotation alone does NOT persist — always go through silk-set/modify).
