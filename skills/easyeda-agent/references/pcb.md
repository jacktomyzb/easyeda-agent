...
- `easyeda pcb net-classes [--json]` — print the **net-class → spec track-width ladder** (规范线宽) the daemon uses: `signal` (live default) / `power-branch` (3V3·1V8, ~10mil) / `power-trunk` (+5V, ~15mil) / `high-current` (VBUS·VIN·VBAT, ~20mil) / `gnd` (prefer pour). Roles are classified by net name/voltage (`pcb_netclass.go`); widths are seeded from the live rules and clamped ≥ the fab minimum. `route-short` sizes each net by this table and `pcb check` width-under-spec gates under-sized power tracks. (A block's declared per-net `track_width_mil` overrides the heuristic — phase-2 consumption.)
