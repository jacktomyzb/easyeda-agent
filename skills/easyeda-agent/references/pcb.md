...
- `easyeda pcb layout-lint` — **score placement quality + predict routability BEFORE routing**。Plain mode 的 `--min-gap` 默认仍是电气 clearance,仅供诊断。**Gate mode 已装配感知(#99)**:先 `pcb stage set-assembly --profile hand-solder|reflow`;`--gate` 读取该档案,手焊将间距地板钳到 ≥40mil,任何 tight pair 都失败,再执行 #97 的 `--min-score`(默认60)+`--max-crossings`(默认8)门。通过才持久化 `pre_route_passed`,与 `outline_confirmed` 一起解锁布线。因此“默认约6mil无告警”不再能冒充“适合手焊”。大焊盘 60–80mil 单侧烙铁通道仍需截图/几何审查并记录在布局确认摘要。
