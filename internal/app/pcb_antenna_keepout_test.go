...
// never overlap a pad (else it strands ground pads). Modelled on ceshi's U1 WROOM:
// bbox y[-129,900], pads up to y≈600 → the antenna strip is the top ~300mil.
func TestAntennaKeepoutRect(t *testing.T) {
	pads := [][2]float64{{0, -100}, {300, 0}, {-300, 300}, {300, 600}, {-300, 600}}
	x0, y0, x1, y1, ok := antennaKeepoutRect(-374, -129, 374, 900, pads, 0.3, 20)
	if !ok {
		t.Fatal("expected a keep-out rect for the WROOM top strip")
	}
	// Top strip: above every pad (maxPadY=600), reaching the module top (900).
	if y0 < 600 {
		t.Errorf("keep-out must not reach the pads (maxPadY=600); got y0=%.1f", y0)
	}
	if y1 < 900 {
	...
	_, sy0, _, _, ok2 := antennaKeepoutRect(-374, -129, 374, 900, pads, 0.1, 20)
	if !ok2 || sy0 <= y0 {
	...
	if _, _, _, _, ok3 := antennaKeepoutRect(0, 0, 100, 300, nil, 0, 0); ok3 {
