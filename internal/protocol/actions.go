...
func AllActions() []ActionSpec {
	return []ActionSpec{
		...
		},
		{
			...
			Mutates:     true,
			NeedsWindow: true,
			Description: "Reconfigure existing silkscreen primitive(s) in ONE batch — designator/value ATTRIBUTES and free STRINGS alike. Provide primitiveIds ([] or one) + any of x/y/rotation/fontSize/lineWidth/text; only the given keys change. Uses the reliable .modify(id,props) (NOTE: setState_Rotation alone does NOT persist; a snapshot taken before a document reload shows the OLD orientation — judge by pcb.silk.list/pcb check, not a stale screenshot). This is the batch position/orientation/size adjuster behind fixing bad-placed or non-upright silk; pair with pcb.silk.list (get ids) and pcb check (silkscreen-flipped).",
			Inputs:      []string{"primitiveIds (string or [] , required)", "x optional (mil)", "y optional (mil)", "rotation optional (deg)", "fontSize optional (mil)", "lineWidth optional (mil)", "text optional"},
			Outputs:     []string{"set", "count", "results[].primitiveId", "results[].ok"},
