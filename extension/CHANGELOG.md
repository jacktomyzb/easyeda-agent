...
## [0.8.8-label-pads] - 2026-07-05

### Added
- **`pcb.silk.label_pads`** — label component pads with pin numbers and/or net names. Useful for large connectors (J2, U1) to mark each pin's function. Reads pad coordinates + nets, computes collision-free positions around each pad, creates labels.

## [0.8.7-label-pads] - 2026-07-05

### Improved
- **`pcb.silk.netnames`** — upgraded layout algorithm: now scans 4 directions (right/left/top/bottom) around each pad to find collision-free placement, avoiding other pads and labels. Fallback to next pad if no slot found.

## [0.8.4-label-pads] - 2026-07-05

### Added
- **`pcb.silk.netnames`** — auto-generate network-name silkscreen labels for nets with pads in a rectangular zone. Useful for debugging/probing: reads all nets, filters those in the zone, computes collision-free positions, and creates free silkscreen strings. Supports left/right alignment, exclude-nets list, custom font size/stroke width.
