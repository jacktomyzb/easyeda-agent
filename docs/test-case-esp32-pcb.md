...
| **P6 校验门** | `pcb drc` + `pcb check` | drc 0 fatal、无 Connection Error（全布通）、keep-out 被尊重；**`pcb check` 0 ERROR**（丝印正反 silkscreen-flipped=0、走线压焊盘 track-over-pad=0） |
...
- [ ] **P6** `pcb drc` → **0 fatal**；keep-out / clearance / netlist 全过；且 **`pcb check` 0 ERROR**（**silkscreen-flipped=0** 丝印无正反/放反、**track-over-pad=0** 无走线压焊盘短路；non-orthogonal 自由角度走线尽量清零）
...
- ✅ **P0 import-changes** 已验证（菜单「更新原理图到PCB」= `pcb.import_changes`，真机跑通）——**仅首次建板**。
- ✅ **P0 增量加件 `pcb add-component`（#20，2026-07-01）** 真机验收：`import-changes` 对 API 增量加件是
  no-op（详见 #20），故走 `pcb add-component` 绕过。真机 ceshi：`sch read` 取 U2(CH340) 脚网
  `{5:3V3,3:GND}` + uniqueId `gge9` → typed `pcb add-component --designator U2 --unique-id gge9
  --nets '{"5":"3V3","3":"GND"}'` → PCB 8→9 件、U2 落板、**pad-nets `{3:GND,5:3V3}` 真接上**、
  uniqueId=gge9 链上原理图孪生。放 footprint + `pcb_PrimitivePad.modify` 逐脚赋网 + 飞线，全程真机通。
  工作流：原理图放+接线 → `sch read` → `pcb add-component`。
...
- ✅ **规则感知 + outline-fit（#22/#21，2026-07-01）** 真机验证——route-short/auto-place 不再硬编码，读板子
  live DRC 规则(`pcb.drc.rules`)。真机 ceshi：route-short report `source=live, clearance=4.02, signalW=10,
  powerW=10`（丢掉 20mil 电源硬编码 → **线宽=板子规范,「导线不符合规范」宽度违规消除**）;`pcb outline-fit`
  把板框利用率 **17%→71%**(器件 1132×1030 + 100mil margin)。整脊柱重跑：Safe-Spacing **10→8**、No-Conn 4。
  残留 8 条 Safe-Spacing = U1 周围平行走线互撞(**无避障**,超出「读规则」范围)→ 归 #23 避障 / #24 Freerouting。
  规则解析器 internal/app/pcb_rules.go(mm→mil,字段级回退 JLCPCB 基线,单测 pcb_rules_test.go)。
...
- ✅ **P2 布局 v1.1（卫星转向 + 去耦贴最近同网pad，#14，2026-06-30）** 真机验证：
  ...
  **0 器件重叠**。② 去耦贴最近同网 pad——芯片 GND/VCC 重复多焊盘时取最近的。
  `--no-rotate` 可回退 v1 纯平移。
- ✅ **P2 多芯片间距（#19，2026-06-30）单测验证**：`spaceMains` 把重叠/间距<`--multi-gap`(默认150mil)
  的主芯片摊成左右一排(最左不动)，卫星相对新位置摆位；`--multi-gap 0` 禁用。单测 `TestPlanAutoPlace_MultiChipSpacing`
  (两个 8 脚芯片重叠→推开到 right+gap+half、最左不动)通过。**真机受阻于 import-changes**：给 ceshi 原理图加
  CH340N(U2,8脚,addIntoPcb+接 3V3/GND+保存)后 `pcb import-changes` 不把新器件同步到 PCB(器件数恒 8)——
  与间距算法无关的导入缺口，转 #20 调查。已删 U2 还原 blink fixture。
