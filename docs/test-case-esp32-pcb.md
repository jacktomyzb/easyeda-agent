...
- 🔴 **P3 DSN keepout = 0（#17）**：`getDsnFile` **丢弃 region** —— 天线 keep-out 进不了 DSN，
  **外部 Freerouting 会在天线下走线**。这是 #5 迷宫档产出「能用的板」的前提红灯（启发式档
  `route-short` + 避让 keep-out 的 `pour` 不受影响）。修法：export-dsn 后注入 `(keepout)`（需真 DSN 校准）。
